...
class _PengaturanModalAwalState extends State<PengaturanModalAwal> {
  ...
  _onSubmit() {
    ...
    _pengaturanController.updateMultipleJournal(widget.id, _tanggal.text,
        _transName.text, _selectedAkuns, _debetText, _creditText);
  }
  ...
  _initItem(int k) {
    setState(() {
      ...
      _akuns.add(
          _pengaturanController.journalList[k]['asset_data_name'].toString() +
              " ( " +
              _pengaturanController.journalList[k]['group'].toString() +
              " )");
      _debitRibuan.add(Ribuan.convertToIdr(
          _pengaturanController.journalList[k]['debet'], 0));
      _creditRibuan.add(Ribuan.convertToIdr(
          _pengaturanController.journalList[k]['credit'], 0));
      _debits.add(TextEditingController(
          text: _pengaturanController.journalList[k]['debet'].toString()));
      _credits.add(TextEditingController(
          text: _pengaturanController.journalList[k]['credit'].toString()));
      _selectedAkuns.add(_pengaturanController.journalList[k]['asset_data_id']
              .toString() +
          '_' +
          _pengaturanController.journalList[k]['account_code_id'].toString());
    });
    _countTotal();
  }
