...
class ProfitLossController extends GetxController {
  ...
  void getProfitLoss() async {
    ...
    if (user != null) {
      ...
      var res = await Network().post(data, '/journal/profit-loss');
      var body = jsonDecode(res.body);
      if (body['success']) {
        profitLoss.value = body['data'];
        loading(false);
        print(profitLoss);
      }
    }
  ...
  void exportPdf() async {
    ...
    if (user != null) {
      ...
      var res = await Network().post(data, '/journal/profit-loss-pdf');
      var body = jsonDecode(res.body);
      if (body['success']) {
        launchURL(Constant.JOURNAL_REPORT + body['data'].toString());
      }
